...
public class MyStorage {
    ...
    static void prod_input(Scanner s) {
        ...
        System.out.println("더 이상 제품을 추가하실 수 없습니다.");
    }
    ...
    static void prod_remove(Scanner s) {
        // 제품 등록 취소 로직을 구현합니다. 입력받은 제품명에 해당하는 배열 요소를 EMPTY로 설정합니다.
            ...
            System.out.printf("[Warning] %s 제품은 등록되어있지 않습니다.%n", productName);
    }
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        if (foundIdx < 0) {  // 제품을 찾지 못한 경우
            System.out.println("[Warning] 입력한 제품명이 없습니다.");
            System.out.println("[Warning] 다시 확인하여 주세요.");
            return;
        }
        ...
        int cnt = Integer.parseInt(quantity);  // 수량 입력 받기
            ...
            for (int i = 0; i < products.length; i++) {
                if (products[i].equals(productName)) {
                    ...
                    for (int j = 0; j < quantity.length(); j++) {
                        if (quantity.charAt(j) < 48 || quantity.charAt(j) > 57) {
                            System.out.println("[Error] 수량은 숫자로 입력해야 합니다.");
                            return;
                        }
                    }
                    if (Integer.parseInt(quantity) > counts[i]) {
                        System.out.println("[Error] 출고 개수를 초과하였습니다.");
                        return;
                    } else if (Integer.parseInt(quantity) <= 0) {
                        System.out.println("[Error] 0보다 큰 수를 입력해주세요.");
                        return;
                    }
                    counts[i] -= Integer.parseInt(quantity);
                    System.out.println("[Clear] 출고가 완료되었습니다.");
                    prod_search();
                    return;
                }
            }
        System.out.printf("[System] %s 제품은 등록되어있지 않습니다.%n", productName);
